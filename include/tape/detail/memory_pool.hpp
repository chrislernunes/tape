...
#ifndef NDEBUG
        if (slot < slots_.data() || slot >= slots_.data() + PoolSize) {
            std::fprintf(stderr, "MemoryPool: free_head_ %p outside slab\n", static_cast<void*>(slot));
            std::abort();
        }
        const std::size_t idx = static_cast<std::size_t>(slot - slots_.data());
        if (in_use_[idx]) {
            std::fprintf(stderr, "MemoryPool: double-alloc idx=%zu\n", idx);
            std::abort();
        }
        if (allocated_ >= PoolSize) {
            std::fprintf(stderr, "MemoryPool: allocated_ overflow\n");
            std::abort();
        }
        in_use_[idx] = 1;
#endif
...
#ifndef NDEBUG
        if (slot < slots_.data() || slot >= slots_.data() + PoolSize) {
            std::fprintf(stderr, "MemoryPool: destroy of non-slab ptr %p\n", static_cast<void*>(ptr));
            std::abort();
        }
        const std::size_t idx = static_cast<std::size_t>(slot - slots_.data());
        if (!in_use_[idx]) {
            std::fprintf(stderr, "MemoryPool: double-free idx=%zu\n", idx);
            std::abort();
        }
        if (allocated_ == 0) {
            std::fprintf(stderr, "MemoryPool: allocated_ underflow\n");
            std::abort();
        }
        in_use_[idx] = 0;
#endif
