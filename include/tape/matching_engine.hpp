...
public:
    ...
    void process(const OrderRequest& req) {
        // Callbacks may submit another order while handle_new() still holds
        // match() results as raw maker Order*. A nested market can finish
        // and destroy a maker the outer emit_trade loop will destroy again.
        // Queued work is FIFO. The flag is RAII-cleared so it cannot stick
        // if dispatch_one throws.
        if (in_process_) {
        ...
        while (!pending_.empty()) {
            OrderRequest next = pending_.front();
...
private:
    ...
    void dispatch_one(const OrderRequest& req) {
        switch (req.action) {
    ...
    // ── Trade emission ────────────────────────────────────────────────────────
    void emit_trade([[maybe_unused]] LimitOrderBook* bk,
    ...
    std::vector<OrderRequest> pending_;
};
