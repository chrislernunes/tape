...
struct BenchmarkResult {
    ...
    void print_json() const {
        const_cast<LatencyHistogram&>(latency_ns).finalize();
        std::printf(
            "{\"name\":\"%s\",\"iterations\":%llu,\"wall_sec\":%.6f,"
            "\"throughput_mops\":%.6f,\"latency_ns\":{"
            "\"min\":%lld,\"p50\":%lld,\"p95\":%lld,\"p99\":%lld,"
            "\"p999\":%lld,\"max\":%lld,\"mean\":%.3f}}\n",
            name.c_str(),
            static_cast<unsigned long long>(iterations),
            wall_sec, throughput_mops,
            static_cast<long long>(latency_ns.min()),
            static_cast<long long>(latency_ns.percentile(50)),
            static_cast<long long>(latency_ns.percentile(95)),
            static_cast<long long>(latency_ns.percentile(99)),
            static_cast<long long>(latency_ns.percentile(99.9)),
            static_cast<long long>(latency_ns.max()),
            latency_ns.mean());
    }
};
...
BenchmarkResult bench_lob_insert_cancel(uint64_t N = 1'000'000) {
    ...
    for (uint64_t i = 0; i < N; ++i) {
        ...
        if (live_orders.size() >= 100) {
            for (int k = 0; k < 50; ++k) {
                [[maybe_unused]] bool cancelled = book.cancel_order(live_orders[k]->id);
...
// ─────────────────────────────────────────────────────────────────────────────
void run_all_benchmarks(bool json = false) {
    if (!json) {
        std::printf("\n╔══════════════════════════════════════════════════╗\n");
        std::printf("║        Tape Benchmark Suite            ║\n");
        std::printf("╚══════════════════════════════════════════════════╝\n");
        std::printf("  Clock: Clock::now_mono()  flags: see docs/BENCHMARKS.md\n");
    }

    auto r1 = bench_lob_insert_cancel(1'000'000);
    auto r2 = bench_matching_engine(500'000);
    auto r3 = bench_memory_pool(2'000'000);
    auto r4 = bench_best_price_query(5'000'000);

    if (json) {
        r1.print_json(); r2.print_json(); r3.print_json(); r4.print_json();
        return;
    }
