...
public:
    NoiseTraderAgent(ClientId cid, InstrumentId iid,
        ...
        , rng_(seed)
        , arrival_rate_per_sec_(arrival_rate_per_sec)
        , arrival_dist_(std::max(arrival_rate_per_sec, 1e-12) / 1e9)
        , side_dist_(0, 1)
        , qty_dist_(1, 5)
    {}

    [[nodiscard]] double arrival_rate() const noexcept { return arrival_rate_per_sec_; }
    [[nodiscard]] uint64_t market_intents() const noexcept { return market_intents_; }
    ...
    void on_market_data(const MarketDataUpdate& mdu) override {
        ...
        if (in_action_ || !clock_) return;

        const Timestamp now = clock_->now();
        if (next_event_ns_ < 0)
            next_event_ns_ = now + static_cast<Timestamp>(arrival_dist_(rng_));

        while (now >= next_event_ns_) {
            in_action_ = true;
            ...
            ++market_intents_;
            in_action_ = false;
            next_event_ns_ += static_cast<Timestamp>(arrival_dist_(rng_));
            if (arrival_rate_per_sec_ <= 0.0) break;
        }
    }
...
private:
    ...
    Timestamp next_event_ns_{-1};
    uint64_t  market_intents_{0};
...
public:
    ...
    void on_market_data(const MarketDataUpdate& mdu) override {
        ...
        if (mdu.type == MarketDataUpdate::Type::L1Quote) {
            last_bid_ = mdu.l1.bid_px;
            last_ask_ = mdu.l1.ask_px;
            return;
        }
        if (mdu.type != MarketDataUpdate::Type::Trade) return;
        ...
        if (last_bid_ <= 0 || last_ask_ <= 0) return;
    ...
    [[nodiscard]] Price last_bid() const noexcept { return last_bid_; }
    [[nodiscard]] Price last_ask() const noexcept { return last_ask_; }
