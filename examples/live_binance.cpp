// Tape — Live Binance Feed (simple scrolling output)
...
int main(int argc, char** argv) {
    ...
    std::printf("\n=================================================================\n");
    std::printf("  Tape | Binance Live Feed | %s\n", symbol_upper.c_str());
    std::printf("=================================================================\n");
    std::printf("  Connecting to stream.binance.com:9443 ...\n\n");
    std::fflush(stdout);

    tape::LocalOrderBook book(symbol_upper);
    tape::LiveAnalytics  analytics;
    tape::BinanceStream  parser;

    uint64_t msg_count  = 0;
    uint64_t print_every = 5;   // print one line every 20 messages (~2 sec)

    parser.set_depth_cb([&](const tape::BinanceDepthUpdate& upd) {
        book.apply_update(upd);
        analytics.on_depth(book);
    });

    parser.set_trade_cb([&](const tape::BinanceTrade& t) {
