...
- `MarketImpactModel` and `CancellationRaceModel` match their own
  ...
  error.
- The full `tape --sim` demo's behavior-relevant output (fills, trades,
...
(`TAPE_BUILD_TESTS=OFF`), matching this project's existing convention for
optional dependencies (same pattern as `BUILD_PYTHON_BINDINGS`); opt in
explicitly:

```
cmake -B build -DTAPE_BUILD_TESTS=ON -DCMAKE_BUILD_TYPE=Debug
cmake --build build -j$(nproc)
ctest --test-dir build --output-on-failure
```

9 files under `tests/`, one binary (`tape_tests`), 57 `TEST_CASE`s, 3,675
assertions:
...
exercised with more than one such file included per binary — `tape`,
`tape_bench`, and `tape_live` are each built from exactly one `.cpp` that
...
`-DTAPE_BUILD_TESTS=ON`, runs `ctest --output-on-failure`, and — Release
leg only — an extra end-to-end check that runs the actual `tape --sim`
...
- **Phase 2:** real `.hpp`/`.cpp` separation, a `tape_core` library target,
...
- **Phase 5:** the offline example and parameter-externalization goals are
  largely already true today (`tape --sim` runs with no network access;
  `tape_live` is already conditionally compiled out without OpenSSL) —
