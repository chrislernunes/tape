...
#  define TAPE_INVALID_SOCKET INVALID_SOCKET
#  define tape_close_socket(s) ::closesocket(s)
#  define tape_sock_error() WSAGetLastError()
#else
...
#  include <fcntl.h>
   using socket_t = int;
#  define TAPE_INVALID_SOCKET (-1)
#  define tape_close_socket(s) ::close(s)
#  define tape_sock_error() errno
#endif
...
public:
    ...
    bool connect(const std::string& host, const std::string& path,
        ...
        if (sock_ == TAPE_INVALID_SOCKET) { freeaddrinfo(res); error("socket()"); return false; }
    ...
    void disconnect() {
        ...
        if (sock_ != TAPE_INVALID_SOCKET) {
            tape_close_socket(sock_);
            sock_ = TAPE_INVALID_SOCKET;
        }
    }
...
private:
    ...
    SSL*      ssl_{nullptr};
    socket_t  sock_{TAPE_INVALID_SOCKET};
